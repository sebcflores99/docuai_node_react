...
Agentic - AI First development, using Claude Opus 4.8 in VsCode Copilot.
...
```bash
make init      # first run: copies .env from the template, builds images, starts everything (detached)
# open http://localhost:3000  → sign up → add a document → ask questions
make logs      # follow container logs
make down      # stop and remove containers
```

`make init` copies `.env.example` to `.env` (if one doesn't exist) and starts the stack. The template defaults to **mock** LLM + embeddings, so it runs offline with no API keys. After the first run, use `make up` to start and `make down` to stop. Migrations are applied automatically on backend startup.

Available targets: `init`, `build`, `up`, `down`, `logs`.
To wipe data (reset Postgres + the vector store), use Docker directly: `docker compose down -v`.
...
The Docker stack reads a single root `.env`. `make init` copies `.env.example` to `.env` for mock mode; to use real models, set the keys and switch providers:
...
# Auth
JWT_SECRET=your-secret-here     # any non-empty value works for local mock mode
JWT_EXPIRES_IN=7d
...
A single root **`.env`** (gitignored) holds shared secrets and is injected by Docker Compose. The committed **`.env.example`** is a template; `make init` copies it to `.env` for local demos.
...
**Backend won't start** — `docker compose logs -f backend` (or `make logs`); ensure Postgres/Weaviate are healthy (`docker compose ps`). Migrations run on startup.
